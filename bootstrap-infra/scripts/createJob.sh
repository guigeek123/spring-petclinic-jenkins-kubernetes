...
TEMPLATE_PATH=seedjobs/templateJobConfig.xml
...
display_usage_and_exit() {
  ...
  echo "-b OPTIONAL: Git branch name configured in the job within, default 'master'" >&2
  exit 1
}
...
configure_template() {
  ...
  sed -i "s#BRANCHNAME#${branch}#" ${path}
}
