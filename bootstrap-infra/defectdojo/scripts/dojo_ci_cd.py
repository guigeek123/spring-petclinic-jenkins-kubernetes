...
Updated by Guigeek123 to manage Clair scan
Updated by Guigeek123 to automatically create product if it doesn't exists in defectdojo (required appropriates access rights in defectdojo)
...
def processFiles(dd, engagement_id, file, scanner=None, build=None):
    ...
    if test_id.success == False:
        print "Upload failed: Detailed error message: " + test_id.message #Modif Guigeek123 : before = test_id.data
...
class Main:
    if __name__ == "__main__":
        ...
        parser.add_argument('--product', help="DefectDojo Product Name in DefectDojo", required=True)
        ...
        if dir is not None or file is not None:
            ...
            product_id = get_or_create_product(dd, product_name)
